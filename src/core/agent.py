...
def build_llm_writer_prompt(item: dict, original_text: str) -> list[dict]:
    """构建 LLM 写作 prompt——基于原文全文重写，而非基于摘要/关键词拼接。"""
    ...
    system = (
        "你是一名资深技术编辑，你的任务是将一篇技术文章用你自己的话重写成一篇公众号推文。\n\n"
        "核心原则——请严格遵守：\n"
        "1. 你必须通读下面的「原文全文」，理解其全部内容后再动笔\n"
        "2. 重写不等于翻译或缩写——你需要用自己的语言重新组织信息，但保留原文中的具体事实\n"
        "3. 从原文中提取具体事实作为文章骨架：人名、项目名、数据、时间、技术术语、具体功能\n"
        "4. 每个二级标题下必须有原文中的具体信息支撑，不要写成空泛的「XX的意义」「XX的启示」\n"
        "5. 段落要自然——事实→解释→判断，不要用「首先其次最后」串段落\n"
        "6. 语言像技术博客，不像新闻稿也不像论文摘要\n"
        "7. 只写原文里有的内容，不要脑补，信息不足就降低判断强度，但绝不要编造\n\n"
        "标题风格（重要）：\n"
        "- 标题不要用「xxx：xxx」这种冒号分隔的格式，太死板\n"
        "- 标题可以是一个完整的观点句、一个反问、一个结论，或者直接点名核心事物\n"
        "- 好的标题例子：「Claude 开始自己写代码了」「MCP 协议到底解决了什么问题」\n"
        "- 差的标题例子：「AI Agent：从概念到实践的演进」「LLM推理优化：技术路线与未来趋势」\n\n"
        "写作语气：\n"
        "- 像你给同行写微信消息一样——有判断、有态度，不端着\n"
        "- 可以用「有意思的是」「说实话」「问题在于」这种自然的口语连接词\n"
        "- 不要每段都以主题句开头然后展开论证，有时直接说事更好\n"
        "- 可以适当表达惊讶、怀疑、期待，但不要夸张\n\n"
        ...
        "- 任何「不是…而是…」的对比句式作为开场\n"
        "- 「本文」「笔者」「小编」等指代自己的词\n"
        "- 「不仅仅…更…」「从…到…」「不仅…还…」这类万能过渡句\n"
        "- 每段开头用「事实上」「实际上」「可以看出」「由此可见」\n"
    )
    ...
    user = f"""请通读下面的原文全文，然后用你自己的语言重写成一篇中文技术公众号文章。

{type_hint}
格式要求：
- 一级标题作为文章标题，要像一个独立文章的标题（不要照搬原标题，不要用冒号分隔的格式）
- 至少 3 个二级标题，每个二级标题要有信息量，但不要用「xxx：xxx」的模板格式
- 正文 1300-1800 字
- 段落 2-4 句，不单句成段
- 至少一处自然的具体例子或类比
{github_note}
原标题（仅供参考，不要照搬）：{source_title}
...
def review_article(
    ...
    system = (
        ...
        "- 如果标题用了「xxx：xxx」这种冒号分隔格式，扣分并建议改成更自然的表达\n"
        "- 如果文章语气像教科书或论文，扣分并建议增加口语化的过渡\n"
...
def revise_article_with_feedback(
    ...
    if round_num >= 3:
        ...
        system = (
            ...
            "替换所有模板化表达、把死板的标题改得更自然。不要删除原文中有价值的事实信息。"
        )
    elif round_num >= 2:
        temp = 0.7
        system = (
