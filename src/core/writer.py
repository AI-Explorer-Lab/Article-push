...
def build_llm_writer_prompt(item: dict, original_text: str) -> list[dict]:
    ...
    v2.4 改进：简化指令、强化核心要求、让 LLM 从原文中提取具体细节而非套用模板。
    """
...
def polish_llm_article(content: str, article_type: str) -> str:
    ...
    v2.4 改进：只做必要的去模板腔替换，不再粗暴删除正常连接词。
    保留「首先」「其次」等正常段落引导词——它们本身没问题，
    问题是滥用时才需要处理。
    """
    replacements = {
        # 模板腔替换
...
def build_intro(article_type: str, subject: str, claim: str, facts: list[str]) -> str:
    ...
    v2.4 改进：简化模板，避免过长的预设句。
    """
    first_fact = fact_or_fallback(facts, 0, f"原文围绕「{subject}」给出了一个具体进展。")
    if article_type == "工具型":
        return (
            f"围绕「{subject}」这个项目，先看它解决什么任务，再看工程上有什么取舍。"
            f"{first_fact}"
        )
    if article_type == "解读型":
        return (
            f"这条动态可以从技术信号的角度来读。{first_fact}"
        )
    return (
        f"{first_fact}"
    )
